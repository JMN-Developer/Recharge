        ...
        if ($request->type=='datatable') {
            ...
            if(Auth::user()->role != 'reseller')
            {
                 if(Auth::user()->role == 'admin'){
                    ...
                    if($request->retailer == 'all')
                    {
                    $data = TransactionHistory::where('parent_id',$user_id)->whereBetween('created_at', [$start_date, $end_date])->latest()->select(['*']);
                    }
                    else
                    {
                        $user_id = $request->retailer;
                        $data = TransactionHistory:: where('parent_id',$user_id)-> where('reseller_id',$user_id)-> whereBetween('created_at', [$start_date, $end_date])->latest()->select(['*']);
